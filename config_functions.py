...
async def generate_config(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="3 Mесяца - 900 рублей", callback_data="product_b")],
        [InlineKeyboardButton(text="6 Месяцев - 1500 рублей", callback_data="product_с")],
...
from modules.info_functions import help_message, about, privacy, terms, limitations, support, whatsnew, status, tutorial
...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        menu_keyboard = [
            ...
            [KeyboardButton("🛒 Купить доступ"), KeyboardButton("📥 Скачать приложение")],
            [KeyboardButton("🔒 Конфиденциальность"), KeyboardButton("⚖️ Условия использования")],
        ...
        await update.message.reply_text("🤖 **DataGuard - Главное меню**\n\nВыберите действие:")

async def handle_menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка нажатий кнопок меню"""
    try:
        text = update.message.text
        print(f"Получена кнопка: {text}")  # Для отладки
